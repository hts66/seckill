...
所以投递延迟是**双峰**的，两种路径差 40 倍：

| 路径 | 实测 p50 | 实测占比 |
|---|---|---|
| 同实例（本地房间广播） | **5.6 ms** | 59.5% |
| 跨实例（等 flush + MQ 扇出） | **204.1 ms** | 40.5% |

> 跨实例那组的 p50 = 204ms，几乎正好等于 `app.chat.flush-interval-ms: 200`——
> **跨实例延迟完全由批量落库的 flush 间隔主导**，RabbitMQ 本身只占零头。
> 想改善跨实例时延，调这个参数比换 MQ 有效得多。

**这意味着：只连一个实例压测毫无意义**（本地广播是 `ConcurrentHashMap` 遍历，测不出东西）。
必须经网关跑，让负载均衡把连接散到两个实例上。
